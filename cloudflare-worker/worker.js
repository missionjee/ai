...
// ==============================================================================
// 5. MAIN PREDICTION ENGINE (GPT 6 ASTRA Quantum Ultra Enterprise Core)
// ==============================================================================
...
async function settlePastDrawsInSupabase(upstreamDraws, supabaseHistory) {
    ...
    // Actively query Supabase for up to 100 recent unsettled signals to guarantee zero dropped settlements
    const unsettledInDb = new Set();
    try {
        const res = await fetch(`${CONFIG.SUPABASE_URL}/rest/v1/global_signals?actual_result=is.null&select=issue_number&order=issue_number.desc&limit=100`, {
            headers: {
                "apikey": CONFIG.SUPABASE_KEY,
                "Authorization": `Bearer ${CONFIG.SUPABASE_KEY}`
            }
        ...
        if (res.ok) {
            ...
            if (Array.isArray(rows)) {
                rows.forEach(r => { if (r && r.issue_number) unsettledInDb.add(String(r.issue_number)); });
            }
        }
    } catch (e) {}

    // Filter strictly for real upstream draws that have a valid lottery number
    const pendingSettlement = upstreamDraws.filter((d, idx) => {
        if (!d || !d.issue_number) return false;
        const k = String(d.issue_number);
        const isRecent = idx <= 4;
        if (!isRecent && alreadySettled.has(k) && !unsettledInDb.has(k)) return false;
    ...
    // Settle newly drawn rounds in parallel (expand slice up to 100 draws)
    const patches = pendingSettlement.slice(0, 100).map(draw => {
...
async function executeSyncCycle(requestedPeriod = null) {
            ...
            if (checkRes.ok) {
                ...
                if (Array.isArray(rows) && rows.length > 0 && rows[0].predicted_type) {
                    ...
                    const cached = {
                        ...
                        status: row.status,
                        tier: row.is_sniper ? "SNIPER" : (row.stake_units === "0U" ? "PASS" : "STANDARD"),
                        recommendedStake: row.stake_units || (row.is_sniper ? "2U" : "1U"),
                        pattern: row.pattern,
    ...
    const supabaseHistory = await hydrateHistoryFromSupabase(500);
        ...
        if (checkExisting.ok) {
            ...
            if (Array.isArray(rows) && rows.length > 0 && rows[0].predicted_type) {
                ...
                const cached = {
                    ...
                    confidence: row.confidence,
                    status: row.status,
                    tier: row.is_sniper ? "SNIPER" : (row.stake_units === "0U" ? "PASS" : (row.stake_units === "4U" ? "MAX-COVER-L3" : (row.stake_units === "2U" ? "RECOVERY-L2" : "STANDARD"))),
                    recommendedStake: row.stake_units || (row.is_sniper ? "2U" : "1U"),
    ...
    else if (trailingLosses >= 2) recoveryLevel = 1; // Hard Stop-Loss Reset! Capped strictly at Level 2 (max -3U drawdown shield).
    ...
    async fetch(request, env, ctx) {
        ...
        if (url.pathname === "/health") {
            return new Response(JSON.stringify({
                ...
                engine: "GPT 6 ASTRA Quantum Ultra Enterprise (Tri-Proxy Fallback + Continuous FIFO Buffer + Supabase Sync)",
                engine_version: "gpt 6 astra",
        ...
        if (url.pathname === "/signal" || url.pathname === "/run" || url.pathname === "/sync" || url.pathname === "/cron") {
            ...
            return new Response(JSON.stringify({
                status: "ONLINE",
                platform: "Cloudflare Workers 24/7",
                engine: "GPT 6 ASTRA Quantum Ultra Enterprise (Tri-Proxy + 5k Continuous FIFO Buffer)",
                engine_version: "gpt 6 astra",
                historical_rounds_buffered: engine.historyBuffer.size,
                diagnostics_url: "/report",
                data: syncResult
...
export { calculateNextPeriod, getUtcCurrentPeriod, CONFIG, fetchWithTriProxy, executeSyncCycle };
